...
ENV MYSQL_PASSWORD zxc12152208910CY
ENV DATABASE_NAME yutou-music
...
# 将构建产物jar包拷贝到运行时目录中
COPY --from=build /app/target/yutou-music-0.0.1.jar .

# 暴露端口
EXPOSE 80

# 执行启动命令
CMD ["java", "-jar", "/app/yutou-music-0.0.1.jar"]